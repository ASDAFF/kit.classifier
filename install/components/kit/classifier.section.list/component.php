...
if(!isset($arParams["PROPERTY_CLASSIFIER_NAME"]) || trim($arParams["PROPERTY_CLASSIFIER_NAME"]==''))
	$arParams['PROPERTY_CLASSIFIER_NAME'] = 'categ_id';

$arParams["IBLOCK_TYPE"] = trim($arParams["IBLOCK_TYPE"]);
$arParams["IBLOCK_ID"] = intval($arParams["IBLOCK_ID"]);

$arParams["IBLOCK_TYPE_STORAGE"] = trim($arParams["IBLOCK_TYPE_STORAGE"]);
$arParams["IBLOCK_ID_STORAGE"] = intval($arParams["IBLOCK_ID_STORAGE"]);
...
$arParams["NOT_SHOW_NULL_ELEMENTS"] = $arParams["NOT_SHOW_NULL_ELEMENTS"]!="N";

$arParams["ADD_SECTIONS_CHAIN"] = $arParams["ADD_SECTIONS_CHAIN"]!="N"; //Turn on by default

$arResult["SECTIONS"]=array();
//--------------------------------------------------------------
function GetClassifierAllSections($arFilter,$arSelect)
{          
    global $arParams;
    ...
    $categ_id_sub_tmp=Array();
    while($ar_result_ = $rs->GetNext())
    {    
      $categ_id_sub_tmp[]=$ar_result_;
    }   
      
        for ($i=0;$i<count($categ_id_sub_tmp);$i++)   {
            $arFilter = Array(
                              'IBLOCK_ID'=>$arParams["IBLOCK_ID"],
                              'GLOBAL_ACTIVE'=>'Y',
                              '>=LEFT_MARGIN' => $categ_id_sub_tmp[$i]['LEFT_MARGIN'],
                              '<=RIGHT_MARGIN' => $categ_id_sub_tmp[$i]['RIGHT_MARGIN']
                        );
            $db_list = CIBlockSection::GetList(Array("left_margin"=>"asc"),  $arFilter, false);
            while($ar_result = $db_list->GetNext())
            {
              $categ_id_sub[]=$ar_result['ID'];
            }
        }
    
    $categ_id=$categ_id_sub;
    $categ_id=array_unique($categ_id );
    return $categ_id;
}
$arResultNotNULL["SECTIONS"]=array();
/*************************************************************************
			Work with cache
*************************************************************************/
if($this->StartResultCache(false, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups())))
{
	...
	else
	{
		...
		if (intval($arParams["LOW_DEPTH"])>1)
		{
			$arFilter[">="."DEPTH_LEVEL"] = $arParams["LOW_DEPTH"];
			$arFilter["<="."DEPTH_LEVEL"] = $arParams["LOW_DEPTH"]+$arParams["TOP_DEPTH"];
		}
		else
...
//---------------------------foreach($arResult["SECTIONS"] as $arSectionID=>$arSection)

    $categ_id_sub=Array();
    $categ_id=Array();
    $show_categ_subsections=true;    
    
    if ($show_categ_subsections) {
        $arFilter = Array('IBLOCK_ID'=>$arParams["IBLOCK_ID"], 'GLOBAL_ACTIVE'=>'Y', 'ID'=>$arSection["ID"]);
        $categ_id=GetClassifierAllSections($arFilter, $arSelect);    
    }
    $ELEMENT_CNT=0;
    if (count($categ_id)>0)
    {
        $arrFilter=Array('IBLOCK_ID'=>$arParams["IBLOCK_ID_STORAGE"], 'ACTIVE'=>'Y', 'PROPERTY_'.$arParams['PROPERTY_CLASSIFIER_NAME']=>$categ_id);
        if (trim($arParams['SECTION_ID_STORAGE'])!='')
            $arrFilter[]=array('SECTION_ID'=>$arParams['SECTION_ID_STORAGE']);
        if (trim($arParams['SECTION_CODE_STORAGE'])!='')
            $arrFilter[]=array('SECTION_CODE'=>$arParams['SECTION_CODE_STORAGE']); 
        
        $db_list = CIBlockElement::GetList(Array(),  $arrFilter, array('ID') , false, $select=Array('ID'));
        if ($arParams['COUNT_ELEMENTS'])
        {
            while($ar_result = $db_list->GetNext())
            {
            $ELEMENT_CNT+=intval($ar_result["CNT"]);
            $arSection["ELEMENT_CNT"]=$ELEMENT_CNT;
            }
            if (intval($arSection["ELEMENT_CNT"])>0)
                $arResultNotNULL["SECTIONS"][]=$arSection;
        }
        else
        {
	...
	if (!$arParams['NOT_SHOW_NULL_ELEMENTS'] || $ELEMENT_CNT>0) 
